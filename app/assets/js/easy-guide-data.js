(function () {
  ...
  window.AURORA_EASY_GUIDES = [
    ...
    },
    {
      ...
      summary: 'Use Aurora Forge buttons and copy the working texture settings. No command line is required.',
      need: ['Aurora Forge for Windows', 'A backup of the original DDS', 'The DDS or PNG files you want to convert', 'A separate output folder'],
      steps: [
        'Open Tools, then open DDS Converter.',
        'Choose DDS to PNG or PNG to DDS.',
        'Choose individual files, or press Convert a DDS Folder / Convert a PNG Folder.',
        'Choose a separate output folder when Aurora Forge asks.',
        'For PNG to DDS, choose the folder containing untouched same-name original DDS files. Aurora Forge matches their format, dimensions, and mipmaps automatically.',
        'Use a manual fallback format only when there is no matching original and you have verified the correct setting.',
        'Press Convert Files if you selected individual files. Folder conversion begins automatically after its required folders are chosen.',
        'Open the conversion report and check each result.',
    ...
    },
    {
      ...
      steps: [
        ...
        'If a payload has no confirmed name, Aurora Forge can still copy it into an Unresolved folder using a safe hash-based filename. An external reference cannot be copied from the selected CAK because that record contains no payload bytes.',
